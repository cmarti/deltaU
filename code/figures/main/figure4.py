...
if __name__ == "__main__":
    ...
    coverage = np.mean(
        (pred["y"] > pred["ci_95_lower"]) & (pred["y"] < pred["ci_95_upper"])
    )
    ...
    fig, subplots = plt.subplots(
        2, 4, figsize=(1.12 * FIG_WIDTH, FIG_WIDTH * 0.45)
    )
    ...
    print(
        "  Plotting variance explained by interactions of order k=2 and k>2 for pairs of sites"
    )
    axes = subplots[1, 3]
