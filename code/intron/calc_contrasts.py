...
def get_contrast_matrix():
    ...
    for bc in backgrounds:
        ...
        factors = (
            [b2, alleles_target] + [background] * 4 + [alleles_target, b21]
        )
        c = kron(factors)
        columns = [
            f"2{bc[0]}_3{a3}_20{a20}_21{bc[1]}"
    ...
    for bc in backgrounds:
        ...
        b21 = alleles_dict[bc[1]].reshape((4, 1))
        factors = (
            [b2] + [background] * 4 + [alleles_target, alleles_target, b21]
        )
        c = kron(factors)
        columns = [
            f"2{bc[0]}_19{a19}_20{a20}_21{bc[1]}"
...
if __name__ == "__main__":
    ...
    print(
        f"Making contrasts with Local Epistasis Regression model fitted to {dataset_label} data"
    )
    ...
    model = SitesVCregression(
        seq_length=8, alphabet_type="dna", lambdas=lambda_U
    )
    model.set_data(X, y, y_var=y_var)
    contrasts = model.make_contrasts(contrast_matrix)

    print("  Saving contrasts...")
    contrasts.to_csv(f"results/{dataset_label}.ssVC.contrasts.csv")
    print("Done.")
