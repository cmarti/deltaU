...
if __name__ == "__main__":
    ...
    data["18,19"] = [x[-4] + x[-3] for x in data.index]
    data['5'] = [x[3] for x in data.index]
    # print(data.groupby(['5'])['f'].mean())
    # exit()
    ...
    backgrounds = ['GC', 'CG', 'GG','UG']
    # backgrounds = ['GC', 'CG', 'GG', 'AG']
    # backgrounds = ['AU', 'AA', 'UU', 'UA']
    seqs = ['GC', 'GG', 'CC', 'CG']
    # seqs = ['UC', 'UU', 'CC', 'CU']
    # seqs = ['AC', 'AG', 'CC', 'CG']
    # seqs = [''.join(x) for x in product('ACGU', 'CAGU')]
    # seqs = ['AU', 'AG', 'CU', 'CG']
    # seqs = ['AU', 'AC', 'GU', 'GC']
    # seqs = ['UC', 'UG', 'CC', 'CG']
    # seqs = ['AC', 'AA', 'CC', 'CA']
    ...
    for background, axes in zip(backgrounds, subplots):
        ...
        means = background_data.groupby(["18,19"])['f'].mean()
        plot_local_landscape(means, seqs, axes, pos1="$_{18}$", pos2="$_{19}$")
    ...
    print("  Saving figure...")
    fig.supxlabel("Genotype at positions 18 and 19", fontsize=7, x=0.6, y=0.08)
