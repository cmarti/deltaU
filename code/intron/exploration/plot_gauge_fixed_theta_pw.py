...
if __name__ == "__main__":
    ...
    pos_alleles = [f"{p}{a}" for p, a in product(range(seq_length), alphabet)]
    position_labels = [2, 3, 4, 5, 18, 19, 20, 21]

    print(f"Plotting gauge-fixed parameters for {dataset_label} dataset")

    print("  Loading pairwise gauge-fixed parameters...")
    theta_pw = pd.read_csv(
        f"results/{dataset_label}.ler.gauge_fixed_theta_pw.csv", index_col=0
    )
    theta_pw['pos_allele_1'] = [f'{pos}{allele}' for pos, allele in zip(theta_pw['pos1'], theta_pw['allele1'])]
    theta_pw['pos_allele_2'] = [f'{pos}{allele}' for pos, allele in zip(theta_pw['pos2'], theta_pw['allele2'])]
    # print(theta_pw)
    # exit()

    gauges = [["GC", "CG"], ["GT", "TG"], ["AT", "TA"]]
    ...
    for axes_row, gauge_pair in zip(subplots, gauges):
        for axes, gauge in zip(axes_row, gauge_pair):
            ...
            theta_lclc = theta_lclc.to_numpy().reshape(seq_length, 4, seq_length, 4)
            axes, cbar = heatmap_pairwise(
                theta_lclc,
                alphabet=alphabet,
                ax=axes,
                clim=(-1.5, 1.5),
            ...
            #ax.set(xticks=[])
            axes.tick_params(axis='x', length=0, rotation=90, pad=0.5)
            axes.set(title=gauge[0] + r"$_2$" + gauge[1] + r'$_{21}$')
    ...
    print("  Saving figure...")
    fig.tight_layout()
    fig.savefig(f"figures/{dataset_label}.gauge_fixed_theta_pw.png", dpi=300)
