...
if __name__ == "__main__":
    for dataset_name in DATASETS:
        ...
        splits = pd.read_csv(
            f"data/processed/splits/{dataset_name}.splits.csv", index_col=0
        )
        if dataset_name not in list_available_datasets():
            data = pd.read_csv(
                f"data/processed/{dataset_name}.csv", index_col=0
            )
        else:
        ...
        for i, p in zip(splits["i"], splits["p"]):
            ...
            train = pd.read_csv(
                f"data/processed/splits/{dataset_name}.{i}.train.csv",
                index_col=0,
            )
            test = pd.read_csv(
                f"data/processed/splits/{dataset_name}.{i}.test.csv",
                index_col=0,
            )
            ...
            for label, model in models.items():
                ...
                print("    Fitting")
                model.fit(X=X, y=y)
                if label not in ["MEI", "Additive", "Pairwise", "Threeway"]:
                ...
                record = evaluate_predictions(
                    pred, X, X_test, y, y_test, label, p
                )
                ...
                with open(
                    os.path.join(out_dir, f"{dataset_name}.{i}.{label}.json"),
                    "w",
                ) as out:
