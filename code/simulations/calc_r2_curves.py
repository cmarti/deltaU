    ...
    for p in np.geomspace(0.01, 0.99, 10):
        ...
        for _ in range(3):
            ...
            for label, model in models.items():
                ...
                r2_train = pearsonr(f_train_pred, f_train)[0] ** 2  # type: ignore
                rmse_train = np.sqrt(np.mean((f_train - f_train_pred) ** 2))
                r2_test = pearsonr(f_test_pred, f_test)[0] ** 2  # type: ignore
                rmse_test = np.sqrt(np.mean((f_test - f_test_pred) ** 2))
                record = {
                    "p": p,
                    "r2_train": r2_train,
